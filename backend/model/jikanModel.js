...
const verifyUser = async ({ username, password }) => {
  const result = await db.query("SELECT * FROM accounts WHERE username = $1", [
    username,
  ]);
...
const createTeam = async ({ team_name }) => {
  const result = await db.query(
    "INSERT INTO teams (team_name) VALUES ($1) RETURNING id, team_name, created_at",
    [team_name]
  );
  return result.rows[0];
};
...
const addNamedUserToTeam = async ({ teamId, username }) => {
  const result = await db.query("SELECT id FROM accounts WHERE username = $1", [
    username,
  ]);
